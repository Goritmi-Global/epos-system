...
const submitProduct = async () => {
  ...
  if (form.value.supplier_id) fd.append("supplier_id", form.value.supplier_id);
  ...
  // Allergies/Tags
  (form.value.allergies || []).forEach((id, i) => fd.append(`allergies[${i}]`, id));
  (form.value.tags || []).forEach((id, i) => fd.append(`tags[${i}]`, id));

  // Image
  if (form.value.imageFile) {
    const ext = (form.value.imageFile.type?.split("/")?.[1] || "webp").replace("jpeg", "jpg");
...
const editItem = (item) => {
  ...
  const cat = props.categories.find(c => c.name === item.category && !c.parent_id);
  let subcat = null;
  if (cat) {
    subcat = props.categories.find(c => c.name === item.subcategory && c.parent_id === cat.id) || null;
  }
  ...
  form.value = {
    ...
    nutrition: item.nutrition || { calories: 0, fat: 0, protein: 0, carbs: 0 },
    allergies: Array.isArray(item.allergies) ? item.allergies.map(Number) : [],
...
const onDownload = (type) => {
  ...
  const dataToExport = q.value.trim() ? filteredItems.value : inventories.value;
...
const downloadCSV = (data) => {
  try {
    const headers = [
      "Item Name", "Category", "Min Alert", "Unit", "Supplier",
      "Sku", "Description", "Nutrition", "Allergies", "Tags", "Created At", "Updated At",
    ];
    const rows = data.map((s) => {
      let nutritionStr = "";
      if (s.nutrition && typeof s.nutrition === "object") {
        nutritionStr = Object.entries(s.nutrition).map(([k, v]) => `${k}: ${v}`).join("; ");
...
const downloadPDF = (data) => {
  try {
    ...
    const tableColumns = [
      "Item Name", "Category", "Min Alert", "Unit", "Supplier",
      "SKU", "Description", "Nutrition", "Allergies", "Tags",
    ];
...
const downloadExcel = (data) => {
  try {
    ...
    worksheet["!cols"] = [
      ...
      { wch: 15 }, { wch: 30 }, { wch: 40 }, { wch: 25 }, { wch: 25 },
      { wch: 20 }, { wch: 20 },
    ];
              ...
              <div class="d-flex flex-wrap gap-2 align-items-center">
                ...
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu dropdown-menu-end shadow rounded-4 py-2">
                    <li><a class="dropdown-item py-2" href="javascript:void(0)" @click="sortBy = 'stock_desc'">From High to Low</a></li>
                    <li><a class="dropdown-item py-2" href="javascript:void(0)" @click="sortBy = 'stock_asc'">From Low to High</a></li>
                    <li><hr class="dropdown-divider" /></li>
                    <li><a class="dropdown-item py-2" href="javascript:void(0)" @click="sortBy = 'name_asc'">Ascending</a></li>
                    <li><a class="dropdown-item py-2" href="javascript:void(0)" @click="sortBy = 'name_desc'">Descending</a></li>
                  </ul>
                </div>
                ...
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu dropdown-menu-end shadow rounded-4 py-2">
                    <li><a class="dropdown-item py-2" href="javascript:;" @click="onDownload('pdf')">Download as PDF</a></li>
                    <li><a class="dropdown-item py-2" href="javascript:;" @click="onDownload('excel')">Download as Excel</a></li>
                    <li><a class="dropdown-item py-2" href="javascript:;" @click="onDownload('csv')">Download as CSV</a></li>
                  </ul>
                </div>
              </div>
            </div>
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  <tr v-for="(item, idx) in sortedItems" :key="item.id">
                    ...
                    <td>
                      <!-- If you use a zoom component, ensure it's registered/imported -->
                      <!-- <ImageZoomModal :file="item.image_url" :width="50" /> -->
                      <img :src="item.image_url" alt="" style="width:50px;height:50px;object-fit:cover;border-radius:6px;" />
                    </td>
                    <td>{{ money(item.unitPrice || 0, "GBP") }}</td>
                    <td class="text-truncate" style="max-width: 260px">{{ item.category }}</td>
                    <td>{{ item.unit }}</td>
                    <td>{{ item.availableStock ? item.availableStock.toFixed(1) : 0 }}</td>
                    <td>{{ money(item.stockValue || 0, "GBP") }}</td>
                    <td class="text-center">
                      ...
                      <span
                        v-else-if="item.availableStock <= item.minAlert"
                        class="badge bg-warning rounded-pill d-inline-block text-center"
                        style="min-width: 100px"
                      >
                        Low-stock
                      </span>
                      <span v-else class="badge bg-success rounded-pill d-inline-block text-center" style="min-width: 100px">
                        In-stock
                      </span>
                    </td>
                    <td class="text-center">
            ...
            <div class="modal-content rounded-4">
              <div class="modal-header">
                <h5 class="modal-title fw-semibold">
                  {{ processStatus === "Edit" ? "Edit Item" : "Add New Item" }}
                </h5>
                <button
              ...
              <div class="modal-body">
                ...
                <div class="row g-3">
                  ...
                  <div class="col-md-6">
                    ...
                    <Select
                      ...
                      :autoZIndex="true"
                      :baseZIndex="2000"
                      @update:modelValue="form.subcategory_id = null"
                  ...
                  <div class="col-md-6" v-if="subcatOptions.length">
                    ...
                    <Select
                      ...
                      :autoZIndex="true"
                      :baseZIndex="2000"
                      :class="{ 'is-invalid': formErrors.subcategory_id }"
                    />
                  ...
                  <div class="col-md-6">
                    ...
                    <input
                      ...
                      placeholder="e.g., 5"
                    />
                    <small v-if="formErrors.minAlert" class="text-danger">{{ formErrors.minAlert[0] }}</small>
                  </div>
                  ...
                  <div class="col-md-6">
                    ...
                    <Select
                      ...
                      :autoZIndex="true"
                      :baseZIndex="2000"
                      :class="{ 'is-invalid': formErrors.unit_id }"
                    />
                    <small v-if="formErrors.unit_id" class="text-danger">{{ formErrors.unit_id[0] }}</small>
                  </div>
                ...
                <div class="row g-4 mt-1">
                  ...
                  <div class="col-md-6">
                    ...
                    />
                    <small v-if="formErrors.allergies" class="text-danger">{{ formErrors.allergies[0] }}</small>
                  </div>
                  ...
                  <div class="col-md-6">
                    ...
                    />
                    <small v-if="formErrors.tags" class="text-danger">{{ formErrors.tags[0] }}</small>
                  </div>
                </div>
                ...
                <div class="col-md-4">
                  <div class="logo-card" :class="{ 'is-invalid': formErrors.image }">
                    <div class="logo-frame" @click="form.imageUrl && openImageModal()">
                ...
                <div class="mt-4">
                  <button class="btn btn-primary rounded-pill px-5 py-2" :disabled="submitting" @click="submitProduct">
                    {{ processStatus === "Edit" ? "Update Item" : "Add Item" }}
                  </button>
                  <button class="btn btn-secondary rounded-pill px-4 ms-2" data-bs-dismiss="modal" @click="resetForm">
                    Cancel
                  </button>
                </div>
              </div>
            ...
            <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
              <div class="modal-header bg-light">
                <div class="d-flex align-items-center gap-2">
                  ...
                  <div>
                    <h5 class="modal-title fw-semibold mb-0">View Inventory Item</h5>
                    <small class="text-muted" v-if="viewItemRef?.sku">SKU: {{ viewItemRef.sku }}</small>
                  </div>
                </div>
                <button
                  class="absolute top-2 right-2 p-2 rounded-full hover:bg-gray-100 transition transform hover:scale-110"
                ...
                <div v-if="viewItemRef" class="row g-4">
                  ...
                  <div class="col-12 col-md-7">
                    <div class="card border-0 shadow-sm rounded-4">
                      <div class="card-body">
                        <h5 class="fw-semibold mb-3">{{ viewItemRef.name }}</h5>
                        <p class="text-muted small mb-2">SKU: {{ viewItemRef.sku }}</p>

                        <div class="row g-3 small">
                          <div class="col-6">
                            <div class="text-muted">Category</div>
                            <div class="fw-semibold">{{ viewItemRef.category || "—" }}</div>
                          </div>
                          <div class="col-6">
                            <div class="text-muted">Subcategory</div>
                            <div class="fw-semibold">{{ viewItemRef.subcategory || "—" }}</div>
                          </div>
                          <div class="col-6">
                            <div class="text-muted">Unit</div>
                            <div class="fw-semibold">{{ viewItemRef.unit }}</div>
                          </div>
                          <div class="col-6">
                            <div class="text-muted">Min Alert</div>
                            <div class="fw-semibold">{{ viewItemRef.minAlert }}</div>
                          </div>
                          <div class="col-12">
                            <div class="text-muted">Supplier</div>
                            <div class="fw-semibold">{{ viewItemRef.supplier }}</div>
                          </div>
                          <div class="col-12">
                            <div class="text-muted">Description</div>
                            <div class="fw-semibold">{{ viewItemRef.description || "—" }}</div>
                          </div>
                        </div>

                        <hr />
                        <h6 class="fw-semibold">Nutrition (per 100g)</h6>
                        <div class="d-flex flex-wrap gap-2 mt-2">
                          <span class="badge rounded-pill bg-primary text-white">Calories: {{ viewItemRef.nutrition?.calories ?? "—" }}</span>
                          <span class="badge rounded-pill bg-success text-white">Protein: {{ viewItemRef.nutrition?.protein ?? "—" }}</span>
                          <span class="badge rounded-pill bg-warning text-white">Fat: {{ viewItemRef.nutrition?.fat ?? "—" }}</span>
                          <span class="badge rounded-pill bg-secondary text-white">Carbs: {{ viewItemRef.nutrition?.carbs ?? "—" }}</span>
                        </div>

                        <hr />
                        <h6 class="fw-semibold">Tags</h6>
                        <div class="d-flex flex-wrap gap-2">
                          <span v-for="tag in viewItemRef.tags" :key="tag" class="badge rounded-pill bg-info text-white">
                            {{ tag }}
                          </span>
                        </div>
                        ...
                        <div class="d-flex flex-wrap gap-2">
                          <span v-for="allergy in viewItemRef.allergies" :key="allergy" class="badge rounded-pill bg-danger text-white">
                            {{ allergy }}
                          </span>
                        </div>
                      </div>
                    ...
                    <div class="card border-0 shadow-sm rounded-4 h-100">
                      <div class="card-body d-flex flex-column align-items-center justify-content-center">
                        <div v-if="viewItemRef.image" class="w-100">
                          <img :src="viewItemRef.image" alt="Item Image" class="w-100 rounded-3" style="max-height: 260px; object-fit: cover;" />
                        </div>
                      </div>

                      <div class="card-footer bg-transparent small d-flex justify-content-between">
                        <span class="text-muted">Updated On</span>
                        <span class="fw-semibold">{{ viewItemRef.formatted_updated_at }}</span>
                      </div>

                      <div class="card-footer bg-transparent small d-flex justify-content-between">
                        <span class="text-muted">Stock</span>
                        <span :class="['fw-semibold', viewItemRef.stock > 0 ? 'text-success' : 'text-danger']">
                          {{ viewItemRef.stock }}
                        </span>
                        <span v-if="viewItemRef.stock === 0" class="badge bg-red-600 rounded-pill">Out of stock</span>
                        <span v-else-if="viewItemRef.stock <= viewItemRef.minAlert" class="badge bg-warning rounded-pill">Low-stock</span>
                        <span v-else class="badge bg-success rounded-pill">In-stock</span>
                      </div>

                      <div class="card-footer bg-transparent small d-flex justify-content-between">
                        <span class="text-muted">Added By</span>
                        <span class="fw-semibold">{{ viewItemRef.user?.name }}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="modal-body">
                <div class="row g-3">
                  ...
                  <div class="col-md-6">
                    ...
                    <input type="number" v-model="stockForm.available_quantity" class="form-control" readonly />
                  </div>

                  <div class="col-md-4">
                    <label class="form-label">Quantity</label>
                    <input type="number" v-model="stockForm.quantity" class="form-control" min="0" @input="calculateValue()" />
                  </div>

                  <div class="col-md-4">
                    <label class="form-label">Price</label>
                    <input type="number" v-model="stockForm.price" class="form-control" min="0" @input="calculateValue()" />
                  </div>
                ...
                <div class="mt-4 text-end">
                  ...
                  <button class="btn btn-secondary ms-2" data-bs-dismiss="modal" @click="resetStockForm">
                    Cancel
                  </button>
                </div>
              </div>
...
/* KPI cards */
.icon-wrap { font-size: 2rem; color: var(--brand); }
...
.search-wrap .bi-search { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: #6b7280; font-size: 1rem; }
...
/* Action menu */
.action-menu { min-width: 220px; }
.action-menu .dropdown-item { font-size: 1rem; }
...
/* Image drop */
.img-drop { height: 160px; border: 2px dashed #cbd5e1; background: #f8fafc; }

/* Optional hint styling */
.dropdown-toggle .small { opacity: 0.85; }

.modal-body .row:not(:last-child) { border-bottom: 1px solid #f0f0f0; padding-bottom: 1rem; margin-bottom: 1rem; }

.badge { font-size: 0.75rem; padding: 0.4em 0.6em; }

.form-label.text-muted { font-size: 0.8rem; margin-bottom: 0.25rem; font-weight: 500; }

.fw-semibold { font-size: 0.95rem; color: #333; }

.table-responsive { overflow: visible !important; }
.dropdown-menu { position: absolute !important; z-index: 1050 !important; }
...
.image-cropper-modal { z-index: 2001 !important; }
.image-cropper-backdrop { z-index: 2000 !important; }
</style>
