...
const fetchAllocations = async (logId) => {
    try {
        const { data } = await axios.get(`stock_entries/stock-logs/${logId}/allocations`);
...
const View = async (row) => {
    ...
    const modal = new bootstrap.Modal(document.getElementById("viewLogModal_out"));
    modal.show();
};

function Edit(row) {
    editForm.value = { ...row };
    const modal = new bootstrap.Modal(document.getElementById("editLogModal_out"));
    modal.show();
}
...
async function updateLog() {
    ...
    try {
        ...
        emit("updated");
        const modal = bootstrap.Modal.getInstance(document.getElementById("editLogModal_out"));
        if (modal) modal.hide();
    } catch (err) {
                            ...
                            <div class="col-lg-12">
                                <div class="card border-0 shadow-sm rounded-4 h-100">
                                    <div class="card-body">
                                        ...
                                        <!-- Allocation table -->
                                        <hr class="my-4" />
                                        <h6 class="fw-semibold mb-3">Stock-out Allocation</h6>
                                        <div class="table-responsive">
                                            <table class="table table-hover align-middle">
                                                <thead class="small text-muted">
                                                    <tr>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr v-if="selectedAllocations.length === 0">
                                                        <td colspan="5" class="text-center text-muted py-3">No allocation history.</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
