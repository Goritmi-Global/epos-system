...
const typeTextClass = (v) => (v === "stockin" ? "text-success" : v === "stockout" ? "text-danger" : "text-secondary");
...
const View = (row) => {
    ...
    const modal = new bootstrap.Modal(document.getElementById("viewLogModal_in"));
    modal.show();
};
function Edit(row) {
    editForm.value = { ...row };
    const modal = new bootstrap.Modal(document.getElementById("editLogModal_in"));
    modal.show();
}
...
const expiryDateModel = computed({
    get: () => (editForm.value.expiryDate ? fmtDate(editForm.value.expiryDate) : ""),
...
async function updateLog() {
    ...
    try {
        ...
        emit("updated");
        const modal = bootstrap.Modal.getInstance(document.getElementById("editLogModal_in"));
        if (modal) modal.hide();
    } catch (err) {
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    <template v-for="row in filtered" :key="row.id">
                        <tr>
                            <td class="fw-semibold text-capitalize">{{ row.itemName }}</td>
                            ...
                            <td>
                                <template v-if="row.expiryDate">
                                    <span
                                        :class="['badge','rounded-pill', expiryBadgeClass(row.expiryDate)]"
                                        :title="
                                            daysUntil(row.expiryDate) !== null
                                                ? daysUntil(row.expiryDate) < 0
                                                    ? Math.abs(daysUntil(row.expiryDate)) + ' day(s) ago'
                                                    : daysUntil(row.expiryDate) + ' day(s) left'
                                                : ''
                                        "
                                    >
                            ...
                            <td class="text-break">
                                <template v-if="row.operationType">
                                    <span :class="['fw-semibold', opTextClass(row.operationType)]">
                            ...
                            <td>
                                <span :class="['fw-semibold', typeTextClass(row.type)]">
                                    {{ formatType(row.type) }}
                                </span>
                            </td>
                            <td class="text-end">
                                <button class="p-2 rounded-full text-blue-600 hover:bg-blue-100" @click="Edit(row)" title="Adjustment">
                                    <Pencil class="w-4 h-4" />
                                </button>
                                <button class="p-2 rounded-full text-gray-600 hover:bg-blue-100" @click="View(row)" title="Show">
                                    <Eye class="w-4 h-4" />
                                </button>
                            </td>
                        </tr>
                    ...
                    <tr v-if="filtered.length === 0">
                        <td colspan="10" class="text-center text-muted py-4">No logs found.</td>
                    </tr>
                </tbody>
            </table>
        </div>
                    ...
                    <div class="modal-header align-items-center">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div class="d-flex flex-column">
                                ...
                                <small class="text-muted">Item: {{ selectedLog?.itemName ?? "—" }}</small>
                            </div>
                        </div>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6 col-md-12">
                                ...
                                <input type="number" class="form-control" v-model="editForm.quantity"
                                       :class="{ 'is-invalid': formErrors.quantity }" required />
                                <small v-if="formErrors.quantity" class="text-danger">{{ formErrors.quantity[0] }}</small>
                            </div>
                            <div class="col-md-6 col-md-12">
                                <label class="form-label">Unit Price</label>
                                <input type="number" class="form-control" v-model="editForm.unitPrice"
                                       :class="{ 'is-invalid': formErrors.unitPrice }" required />
                                <small v-if="formErrors.unitPrice" class="text-danger">{{ formErrors.unitPrice[0] }}</small>
                            </div>
                            <div class="col-md-6 col-md-12">
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <input type="date" class="form-control" v-model="expiryDateModel" />
                            </div>

                            <div class="mt-4">
                                <button class="btn btn-primary rounded-pill px-4 d-inline-flex align-items-center gap-2"
                                        :disabled="isUpdating" :aria-busy="isUpdating ? 'true' : 'false'" @click="updateLog">
