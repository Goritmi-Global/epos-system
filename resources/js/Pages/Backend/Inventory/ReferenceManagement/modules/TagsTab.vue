...
// table data (what you've already saved)
const rows = ref([{ id: 1, name: "Vegan" }, { id: 2, name: "Halal" }]);

// Select options (static/common)
const options = ref([
...
const filterText = ref("");     // what's typed in the filter box
...
const onAdd = () => {
  ...
  selected.value.forEach(v => { if (!names.has(v)) rows.value.push({ id: Date.now()+Math.random(), name: v }); });
  selected.value = [];
};
    ...
    <div class="card-body">
      <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
        ...
        <div class="d-flex gap-2">
          ...
          <button class="btn btn-primary rounded-pill px-4" data-bs-toggle="modal" data-bs-target="#modalAddTags">
            Add Tag
          </button>
        </div>
      </div>
      ...
      <div class="table-responsive">
        <table class="table table-hover">
          <thead class="border-top small text-muted">
            <tr><th>S. #</th><th>Tag</th><th class="text-end">Action</th></tr>
          </thead>
          <tbody>
            <tr v-for="(r,i) in filtered" :key="r.id">
              ...
              <td class="text-end">
                <button class="btn btn-link text-danger p-0" @click="removeRow(r)">Delete</button>
              </td>
            </tr>
            <tr v-if="filtered.length===0"><td colspan="3" class="text-center text-muted py-4">No tags found.</td></tr>
          </tbody>
        </table>
      </div>
    </div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
      <div class="modal-content rounded-4">
        ...
        <div class="modal-body">
          ...
          >
            <template #header>
              <div class="w-100 d-flex justify-content-end">
                <button type="button" class="btn btn-sm btn-link text-primary" @click.stop="selectAll">Select All</button>
              </div>
            </template>
            <template #footer>
              <div v-if="filterText?.trim()" class="p-2 border-top d-flex justify-content-between align-items-center">
                <small class="text-muted">Not found?</small>
                <button type="button" class="btn btn-sm btn-outline-primary rounded-pill" @click="addCustom">
                  Add “{{ filterText.trim() }}”
                </button>
              </div>
            </template>
          ...
          <button class="btn btn-primary rounded-pill w-100 mt-4" @click="onAdd" data-bs-dismiss="modal">Add Tag(s)</button>
        </div>
      </div>
    </div>
  </div>
...
.search-wrap .bi-search{ position:absolute; left:12px; top:50%; transform:translateY(-50%); color:#6b7280; }
.search-input{ padding-left:38px; border-radius:9999px; }
:deep(.p-select){ width:100%; }
:deep(.p-select-token){ margin:.15rem; }
</style>
