...
const filteredOrders = computed(() => {
    ...
    if (!t) return orders.value;
    return orders.value.filter((o) =>
        [o.supplier, o.status, String(o.total), o.purchasedAt]
            .join(" ")
            .toLowerCase()
            .includes(t)
    );
...
const supplierOptions = ref(["Noor", "Wali Khan", "Metro", "Al-Fatah"]);
...
function addPurchaseItem(item) {
    ...
    const price =
        p_price.value !== ""
            ? Number(p_price.value)
            : Number(item.defaultPrice);
    const expiry = p_expiry.value || null;
...
function quickPurchaseSubmit() {
    const payload = {
        supplier: p_supplier.value,
        items: p_cart.value.map(({ id, qty, unitPrice, expiry }) => ({
            id,
            qty,
            unitPrice,
            expiry,
