...
const props = defineProps({
  ...
  btnClass: { type: String, default: "btn btn-primary rounded-pill mt-3 px-4" },
});
...
const isOpen = computed(() => (props.controlled ? props.show : localShow.value));
...
// crop → emit File
function cropImage() {
  const canvas = cropper.value?.getCroppedCanvas?.({ willReadFrequently: true });
  if (!canvas) return;
...
// ESC closes
function onKey(e) { if (e.key === "Escape" && isOpen.value) closeModal(); }
...
<template>
  ...
  <teleport to="body">
    <transition name="fade">
      ...
      >
        <div class="w-full max-w-4xl max-h-[92vh] bg-white rounded-xl shadow-2xl p-5 overflow-auto relative">
          <!-- Header -->
          <div class="flex items-center justify-between gap-3 mb-4">
            <h2 class="text-xl font-semibold">Upload and Crop Image</h2>
            <div class="flex items-center gap-2">
              <button class="btn btn-outline-secondary" @click="openPicker">Change Image</button>
              <button
          ...
          <div class="grid grid-cols-12 gap-4">
            ...
            <div class="col-span-12 md:col-span-3">
              ...
              <div class="cropper-preview border rounded overflow-hidden bg-gray-100"
                   style="width:100%;max-width:220px;height:220px"></div>
            </div>
          </div>

          <!-- Tools -->
          <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mt-4">
            <button class="bg-blue-600 text-white text-sm py-1.5 px-4 rounded" :disabled="!hasImage" @click="rotate(90)">Rotate +90°</button>
            <button class="bg-blue-600 text-white text-sm py-1.5 px-4 rounded" :disabled="!hasImage" @click="rotate(-90)">Rotate -90°</button>
            <button class="bg-blue-600 text-white text-sm py-1.5 px-4 rounded" :disabled="!hasImage" @click="flipX">Flip X</button>
            <button class="bg-blue-600 text-white text-sm py-1.5 px-4 rounded" :disabled="!hasImage" @click="flipY">Flip Y</button>
          </div>

          <!-- Actions -->
          <div class="flex justify-end mt-6 gap-2">
            <button class="bg-gray-200 text-gray-800 px-4 py-2 rounded hover:bg-gray-300" :disabled="!hasImage" @click="reset">Reset</button>
            <button class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700" :disabled="!hasImage" @click="cropImage">Crop & Save</button>
          </div>
        </div>
      </div>
    </transition>
